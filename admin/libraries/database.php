...
<?php 
    ...
    $sql = 'SELECT * FROM phancong pc INNER JOIN chucnang_gv cngv ON pc.id_cn_gv = cngv.id_cn_gv INNER JOIN giaovien gv ON gv.magv = cngv.magv INNER JOIN monhoc mh ON mh.mamh = cngv.mamh INNER JOIN khoi k ON k.makhoi = cngv.makhoi INNER JOIN namhoc nh ON pc.id_namhoc = nh.id_namhoc INNER JOIN lop l ON pc.malh = l.malh ORDER BY pc.mapc DESC';
