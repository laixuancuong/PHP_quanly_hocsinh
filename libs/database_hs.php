...
/*--môn học-----------*/
    $sql = "SELECT cngv.magv, gv.*, nh.namhoc, mh.*, l.* FROM phancong pc INNER JOIN lop l ON l.malh = pc.malh INNER JOIN phancong_cn pccn ON l.malh = pccn.malh INNER JOIN namhoc nh ON nh.id_namhoc = pc.id_namhoc INNER JOIN chucnang_gv cngv ON cngv.id_cn_gv = pc.id_cn_gv INNER JOIN monhoc mh ON cngv.mamh = mh.mamh INNER JOIN giaovien gv ON gv.magv = cngv.magv  INNER JOIN phancong_lh pclh ON pclh.id_pc_cn = pccn.id_pc_cn WHERE pc.id_namhoc = (SELECT MAX(id_namhoc) FROM phancong) AND pccn.id_namhoc = pc.id_namhoc AND pclh.mahs = ".$_SESSION['user_hs']."
                                                ";
    $dl_mon = $conn->query($sql);
/*--Học liệu-----------*/
    $sql = "SELECT * FROM phancong pc INNER JOIN lop l ON l.malh = pc.malh INNER JOIN hoclieu hl ON hl.mapc = pc.mapc INNER JOIN phancong_cn pccn ON l.malh = pccn.malh INNER JOIN namhoc nh ON nh.id_namhoc = pc.id_namhoc INNER JOIN chucnang_gv cngv ON cngv.id_cn_gv = pc.id_cn_gv INNER JOIN monhoc mh ON cngv.mamh = mh.mamh INNER JOIN phancong_lh pclh ON pclh.id_pc_cn = pccn.id_pc_cn
